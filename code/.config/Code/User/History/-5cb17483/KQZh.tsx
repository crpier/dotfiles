    ...
    this.setState(
      {
        ...
        {
          target:
            { type: "House", left: 1, right: 2 },
          popup: { left: 3, right: 3 },
        },
      }
    )
  }
...
function Popup(props: { target: { type: string, left: number, top: number }, popup: { left: number, top: number }, ws: WebSocket, closeHandler: (e: any) => void }) {
  ...
  function createThing(target: {type: string, left:number, top:number}) {
    const eventType = props.target.type == "Grass" ? "create" : "delete"
    const notification = JSON.stringify({
      type: "event",
      event: eventType,
      data: JSON.stringify({ type: target.type, col: target.left, row: target.top })
    })
  ...
  return <div style={{ position: "absolute", top: 96, left: 96, zIndex: 100 }} className="p-2 bg-yellow-500">
    <p>Create Thing</p>
    <ul>
      <li  onClick={createThing({target: {type: "house", left: 3, top: 4}})} className="flex p-1 my-4 hover:bg-yellow-300"><p className="flex-grow">House</p><img className="mx-4" src="house.png"></img></li>
