...
function Thing(props: { left: number, top: number, type: string, ws: WebSocket, closeHandler: (e: any)=>void }) {
    ...
    return <main className="container mx-auto flex flex-col items-center justify-center min-h-screen p-4">
      ...
      {this.state.items?.map((item: any) => (
        <div onClick={this.openPopup}>
          <Thing type={item.type} left={item.loc.col} top={item.loc.row} key={`${item.type}:${item.loc.row}-${item.loc.col}`} ws={this.ws} closeHandler={this.closePopup}/>
        </div>
      ))}
    </main>
  }
...
function Popup() {
  function createThing(props: { target: { type: string, left: number, top: number }, popup: { left: number, top: number }, ws: WebSocket }) {
    const eventType = props.target.type == "Grass" ? "create" : "delete"
  ...
  return <div style={{ position: "absolute", top: 96, left: 96, zIndex: 100 }} className="p-2 bg-yellow-500">
    <p>Create Thing</p>
    <ul>
      <li className="flex p-1 my-4 hover:bg-yellow-300"><p className="flex-grow">House</p><img className="mx-4" src="house.png"></img></li>
